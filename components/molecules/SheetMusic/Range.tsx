...
function runVexFlowRangeCode(
  ...
  for (const [low, high] of keys) {
    const { drawNote: loDrawNote, drawAccident: loDrawAccident } = getDrawNote(
      low,
      keySignature,
      keys as unknown as Note[]
    );
    const { drawNote: hiDrawNote, drawAccident: hiDrawAccident } = getDrawNote(
      high,
      keySignature,
      keys as unknown as Note[]
    );
