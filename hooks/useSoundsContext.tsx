...
const SoundContextProvider = (props: { children: ReactNode }) => {
    ...
    useEffect(() => {
        const loadSounds = async () => {
            ...
            allAssets.forEach(async (asset, i) => {
                try {
                    const key = pluckNoteFromMp3Filename(asset.name);
                    const file = asset.localUri;
                    const audioBuffer = await audioContextRef.current!.decodeAudioDataSource(file!);
                    console.log("audioBuffer :::", audioBuffer);

                    bufferMapRef.current[key] = audioBuffer;

                    const isLastAsset = i + 1 == allAssets.length;
                    if (isLastAsset) {
                        console.log(`<SoundContext> Done loading Piano Sounds. Took ${Date.now() - timeStart}ms`);
                        setReady(true);
                    }
                } catch (error) {
                    console.error("Failed to load or decode the audio file:", error);
                }
            });
        };
